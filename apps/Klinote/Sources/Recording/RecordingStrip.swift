...
//
// The client-visible indicator. This is the most consequential surface in the
...
struct RecordingStripView: View {
    ...
    private var headline: String {
        ...
        default:
            model.captureKind == .dictate
                ? "Dictating the note. Audio stays on this Mac."
                : "Recording this session. Audio stays on this Mac."
        }
    }
            ...
            HStack(spacing: KlinoteMetrics.space8) {
                if model.recordingState.isHolding {
                    ...
                    Button("Hold") { model.holdRecording() }
                        .controlSize(.small)
                        .disabled(isDrafting)
                        .help("Stop recording without ending the session. The held part is not captured and the note says so.")
