...
struct PasteTranscriptSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: KlinoteMetrics.space16) {
            Text("Paste what was said")
                .font(KlinoteFont.panelHeading())
                .foregroundStyle(KlinoteColor.primary)
            Text("One line per turn, starting with CLINICIAN: or PATIENT: — those are the two voices. The letter calls them therapist and client. No audio and no download.")
