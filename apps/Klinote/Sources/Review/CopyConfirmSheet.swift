...
//
// Copy note is one keystroke, and pasting the wrong client's note into a
// record is the failure a therapist would actually fear. Copying what is
...
struct CopyConfirmSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: KlinoteMetrics.space16) {
            Text(isPrinting ? "Print this session?" : "Copy this session?")
                .font(KlinoteFont.panelHeading())
                .foregroundStyle(KlinoteColor.primary)
            Text(isPrinting
                 ? "This is not the session the window was showing. Check it is the right one before it reaches paper."
                 : "This is not the session the window was showing. Check it is the right one before you paste.")
            ...
            VStack(alignment: .leading, spacing: KlinoteMetrics.space8) {
                ...
                Field(label: "Session", value: EncounterSpine.time(encounter.startedAt))
