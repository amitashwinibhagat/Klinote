        ...
        Group {
            if let encounter = model.selectedEncounter,
               ...
               let transcript = encounter.transcript {
                ...
                EmptyState(
                    title: "Paste what was said, or record the next session.",
                    message: "A transcript you already have is enough — no download. Recording needs listening, once.",
                    action: ("Paste a transcript", { model.isPasting = true }),
                    secondary: ("Record this session", { model.startRecording() })
                )
                .padding(KlinoteMetrics.space48)
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .background(KlinoteColor.desk)
...
struct Letterhead: View {
        ...
        VStack(alignment: .leading, spacing: KlinoteMetrics.space12) {
            ...
            if canSwap {
                Button {
                    ...
                    Text(model.isSwapping ? "Swapping voices…" : "Voices look wrong? Swap therapist and client")
                }
                .buttonStyle(.plain)
    ...
    private var templateTitle: String {
        if let name = model.noteTemplates.first(where: { $0.id == note.templateId })?.name {
            return name
        }
        return note.templateId == "psychology" ? "Therapy session note" : "Progress note"
    }
...
struct ProvisionanceLine: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: KlinoteMetrics.inline2) {
            ...
            if isSynthetic {
                Text("Sample text — not a real session.")
                    .font(KlinoteFont.label())
        ...
        .accessibilityElement(children: .combine)
        .accessibilityLabel(isSynthetic ? "\(engineLine) Sample text, not a real session." : engineLine)
    }
}
...
struct NoteModelPrompt: View {
    ...
    private var detail: String {
        ...
        default:
            return "Quire writes a fuller note from the same recording. It downloads once, stays on this Mac, and costs nothing per note. The transcript is already saved, so this note is written again from it — you do not have to record the session again."
        }
    }
}
        ...
        VStack(alignment: .leading, spacing: KlinoteMetrics.space12) {
            ...
            if section.sentences.isEmpty {
                ...
                if !isAdding {
                    ...
                    Text("Quire writes what it heard. Whatever this session did not cover, write here.")
                        .font(KlinoteFont.label())
                        .foregroundStyle(KlinoteColor.tertiary)
                        .fixedSize(horizontal: false, vertical: true)
                }
...
struct NameCheckBlock: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: KlinoteMetrics.space12) {
            ...
            Text("Heard in the session. Not replaced until you say so.")
                .font(KlinoteFont.caption())
                .foregroundStyle(KlinoteColor.secondary)
...
struct UnfiledBlock: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: KlinoteMetrics.space12) {
            ...
            Text("Heard, not placed. File them or drop them before you copy.")
                .font(KlinoteFont.caption())
                .foregroundStyle(KlinoteColor.secondary)
                .fixedSize(horizontal: false, vertical: true)
