...
| Live capture, speech recognition, speaker turns (system `SpeechAnalyzer`) | ✅ on-device, **no model to download** |
| Model-backed note generation (Quire, downloaded once) | ✅ on-device |
...
| `scripts/observe.sh` | After mutating anything outside the repo: prints what the product actually reads. |
| `docs/product/POSITIONING.md` | Recast: therapists, not GPs. Canonical. |
| `docs/product/LAUNCH-READINESS.md` | Why this is not a public launch yet. |
| `docs/product/COMPETITORS.md` | Heidi, Freed, Lyrebird, Tali, Nabla — dated prices. |
| `docs/product/PRICING.md` | $99/clinician-month as a *test*, not a website. |
